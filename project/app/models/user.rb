class User < ApplicationRecord
  ...
  def status_match
    if User.find_by(num: num)
      errors.add('学工号','已被注册')
    end
    errors.add('学工号', '应为6位或8位的数字') if num.length != 8 && num.length != 6
    num.chars.each do |c|
      errors.add('学工号', '应为6位或8位的数字') if c > '9' || c < '0'
    end
    return unless (num.length == 8 && status == 'teacher') || (num.length == 6 && status == 'student')
